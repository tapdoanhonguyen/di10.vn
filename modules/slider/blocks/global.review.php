...
if (!nv_function_exists('nv_block_review')) {
    /**
     * nv_block_config_review()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_review($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_review_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_review_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
     ...
     * @return
     */
    function nv_block_review($block_config)
    {
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file . '/block.review.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $module_file . '/block.review.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block.review.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $module_file);
...
if (defined('NV_SYSTEM')) {
    $content = nv_block_review($block_config);
}