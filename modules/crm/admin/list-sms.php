...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        list( $scenarioid ) = $db->query( 'SELECT id FROM ' . NV_PREFIXLANG . '_' . $module_data . '_smsconfig WHERE id=' . $id )->fetch(3);
        if( $scenarioid > 0 ){
            $count = $db->exec('DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_smsconfig WHERE id =' . $id);
            if ($count) {
                $db->exec("DELETE FROM " . NV_PREFIXLANG . "_" . $module_data . "_sms_queue WHERE smsconfigid=" . $id);
...
$db->sqlreset()
    ->select('COUNT(*)')
    ->from('' . NV_PREFIXLANG . '_' . $module_data . '_smsconfig')
    ->where( $where);
...
$xtpl->assign('add_sms_config', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=smsconfig' );
...
while ($view = $sth->fetch()) {
    ...
    $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=smsconfig&amp;id=' . $view['id'];
