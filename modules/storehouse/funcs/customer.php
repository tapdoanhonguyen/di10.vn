...
if($groups==12 || $groups==1){
	...
	$db->sqlreset()
        ...
        ->where('cu.userid = ' . $user_info['userid'])
		->order('id DESC');

  
    $sth = $db->prepare($db->sql());
    $sth->execute();
	$customer = $sth->fetch(5);
	...
	if(!empty($customer)){
		...
		if($view == 'project'){
			$contents = nv_theme_storehouse_customer_project($customer,$rpproduct,$date_from,$date_to);
		}elseif($view == 'sales' ){
			$contents = nv_theme_storehouse_customer_sales($customer,$rpproduct,$date_from,$date_to);
		}else{
			$contents = nv_theme_storehouse_customer_info($customer,$rpproduct,$date_from,$date_to);
		} 
	}else{
