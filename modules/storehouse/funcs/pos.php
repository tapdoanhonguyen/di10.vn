...
if (! defined('NV_IS_USER')) {
    $redirect = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=pos";
	...
	if($nv_Request->get_int('total_items', 'post') > 0)
	{
		...
		$c_i =0;
		foreach ($array_category_id_storehouse as $key => $cat){
			if($cat['parent_id'] == 0 ){
				$c_i++;
				$cat['no'] = $c_i;
