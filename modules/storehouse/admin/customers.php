...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        ...
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Customers', 'ID: ' . $id, $admin_info['userid']);
    ...
    if (empty($error)) {
        try {
            if (empty($row['id'])) {
                ...
                $stmt = $db->prepare('INSERT INTO ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_companies (group_id, group_name, customer_group_id, customer_group_name, name, code, company, vat_no, address, city, state, postal_code, country, phone, email, cf1, cf2, cf3, cf4, cf5, cf6, invoice_footer, payment_term, logo, award_points, deposit_amount, price_group_id, price_group_name, gst_no, idsite) VALUES (:group_id, :group_name, :customer_group_id, :customer_group_name, :name, :code, :company, :vat_no, :address, :city, :state, :postal_code, :country, :phone, :email, :cf1, :cf2, :cf3, :cf4, :cf5, :cf6, :invoice_footer, :payment_term, :logo, :award_points, :deposit_amount, :price_group_id, :price_group_name, :gst_no, ' .$idsite . ')');
                ...
                $stmt = $db->prepare('UPDATE ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_companies SET customer_group_id = :customer_group_id, customer_group_name = :customer_group_name, name = :name, code = :code, company = :company, vat_no = :vat_no, address = :address, city = :city, state = :state, postal_code = :postal_code, country = :country, phone = :phone, email = :email, invoice_footer = :invoice_footer, logo = :logo, price_group_id = :price_group_id, price_group_name = :price_group_name, gst_no = :gst_no WHERE id=' . $row['id']);
            }
            ...
            if ($exc) {
                ...
                if (empty($row['id'])) {
					...
					if($ok){
					    ...
					    $oks = $db->insert_id($_sql, 'groups_id', $data_insert);
					}
                	...
                	if($users >0 ){
					    ...
					    $ok = $db->insert_id($_sql, 'id', $data_insert);
						if($ok){
							$users_groups = $db->query('SELECT COUNT(userid) FROM ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_groups_user WHERE group_id = 12 AND userid = ' . $row['user'] )->fetchColumn();
							if($users_groups == 0 ){
								$_sql = 'INSERT INTO ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_groups_user (groups_id, userid, is_leader, approved, data ) VALUE (12, :userid, 0 , 1,0 )';
								$data_insert = array();
							    $data_insert['userid'] = $row['user'];
                    ...
                    nv_insert_logs(NV_LANG_DATA, $module_name, 'Edit Customers', 'ID: ' . $row['id'], $admin_info['userid']);
                }
                nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=customers_list');
            }
