...
// Danh sách
$sql = 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_api_role ORDER BY role_id DESC';
$result = $db->query($sql);
...
if ($nv_Request->isset_request('del', 'post')) {
    ...
    $db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_api_role WHERE role_id=' . $role_id);
...
if ($role_id) {
    ...
    $table_caption = $lang_module['api_roles_edit'];
    ...
    $table_caption = $lang_module['api_roles_add'];
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $sql = 'SELECT role_id FROM ' . $db_config['prefix'] . '_' . $module_data . '_api_role WHERE role_title=:role_title' . ($role_id ? (' AND role_id!=' . $role_id) : '');
    $sth = $db->prepare($sql);
    ...
    if (empty($array_post['role_title'])) {
        ...
        if ($role_id) {
            $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_api_role SET
            ...
            $sql = 'INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_api_role (
...
foreach ($array_api_actions as $keysysmodule => $sysmodule_data) {
    ...
    $cat1_total_api = 0;
	$keysysmodules='storehouse';
    // Lev1: Hệ thống hoặc các module
    $xtpl->assign('CAT1_KEY', $keysysmodules);
    $xtpl->assign('CAT1_NAME', $keysysmodules ? $site_mods[$keysysmodules]['custom_title'] : '');
