 ...
 * @Createdate Sat, 11 Aug 2018 03:34:31 GMT
 */
...
if ($nv_Request->isset_request('delete_id', 'get') and $nv_Request->isset_request('delete_checkss', 'get')) {
    ...
    if ($id > 0 and $delete_checkss == md5($id . NV_CACHE_PREFIX . $client_info['session_id'])) {
        ...
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Supply_list', 'ID: ' . $id, $admin_info['userid']);
...
if (!$nv_Request->isset_request('id', 'post,get')) {
	...
	$where = 'group_id IN (2) AND idsite='.$idsite; 
	if (!empty($q)) {
    ...
    if (!empty($q)) {
        ...
        $sth->bindValue(':q_state', '%' . $q . '%');
        $sth->bindValue(':q_country', '%' . $q . '%');
        $sth->bindValue(':q_phone', '%' . $q . '%');
        $sth->bindValue(':q_email', '%' . $q . '%');
        $sth->bindValue(':q_gst_no', '%' . $q . '%');
    }
    $sth->execute();
...
$xtpl = new XTemplate('supply_list.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        $view['number'] = $number++;
        $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=supply&amp;id=' . $view['id'];
