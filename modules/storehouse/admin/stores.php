...
if ($nv_Request->isset_request('ajax_action', 'post')) {
    ...
    if ($new_vid > 0)     {
        ...
        {
            ++$sort_order;
            if ($sort_order == $new_vid)
			++$sort_order;
			$sql = 'UPDATE ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_stores SET sort_order=' . $sort_order . ' WHERE store_id=' . $row['store_id'];
            $db->query($sql);
        }
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    $row['uid'] = $nv_Request->get_int('uid', 'post', '');
    ...
    if (empty($error)) {
        try {
            if (empty($row['store_id'])) {
                ...
                $stmt = $db->prepare('INSERT INTO ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_stores (name, category_id, url, userid, sort_order) VALUES (:name, :category_id, :url, :uid, :sort_order)');
                $stmt->bindParam(':sort_order', $weight, PDO::PARAM_INT);
            } else {
                $stmt = $db->prepare('UPDATE ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_stores SET name = :name, url = :url, category_id = :category_id, userid = :uid WHERE store_id=' . $row['store_id']);
            }
            ...
            if ($exc) {
				...
				$id=$db->query('SELECT setting_id FROM ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_settings WHERE setting_id = ' . $storeid)->fetch(5)->setting_id;
            	if ($id == 0 && $storeid > 0) {
            		$row['logo'] = '';
				    $row['logo2'] = '';
				    $row['site_name'] = '';
				...
				$stores_info = $db->query('SELECT * FROM ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_stores WHERE store_id = ' . $storeid)->fetch(5);
				$user=$db->query('SELECT userid FROM ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_users WHERE userid = ' . $stores_info->userid . '');
				if($user->rowCount() == 0 && $storeid > 0){ 
                	$db->query('INSERT INTO ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_users (userid,storeid,is_staff)  VALUE (' . $stores_info->userid . ',' . $storeid . ' ,1) ');	  
                }else{
                	$db->query('UPDATE ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_users SET storeid = ' . $storeid . ' WHERE userid =' . $stores_info->userid ); 
                } 
				$user_store=$db->query('SELECT userid FROM ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_users_stores WHERE storeid = ' . $storeid . ' AND chain=1'); 
                if($user_store->rowCount() == 0 && $storeid > 0){	 
                	$db->query('INSERT INTO ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_users_stores (userid,storeid,chain)  VALUE (' . $row['uid'] . ',' . $storeid . ',1) '); 
                }else{ 
                	$db->query('UPDATE ' . $db_config['dbsystem'] . '.' . $db_config['prefix'] . '_' . $module_data . '_users_stores SET userid = ' . $row['uid'] . ' WHERE storeid=' . $storeid); 
                }
                $nv_Cache->delMod($module_name);
                if (empty($row['store_id'])) {
...
$xtpl = new XTemplate('stores.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;store_id=' . $view['store_id'];
...
$row['category'] = explode(",",$row['category_id']);
