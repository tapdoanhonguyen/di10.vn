...
if (!$nv_Request->isset_request('id', 'post,get')) {
	...
	$where = 'puidsite = ' . $global_config['idsite'];
		if (!empty($date_from)) {
...
if ($_SESSION[$module_data . '_store_id']>0 && $show_view && $_SESSION[$module_data . '_store_id']!=$parentid || $groups == 1) {
    ...
    while ($view = $sth->fetch()) {
        ...
        $view['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=purchases&amp;id=' . $view['id'];
		...
		if($global_config['idsite'] == 0){
			$xtpl->parse('main.view.loop.main_site_payment');
		}
