...
if($parentid != $_SESSION[$module_data . '_store_id'] && $_SESSION[$module_data . '_store_id'] > 0 || $groups == 1){
	...
	if (!$nv_Request->isset_request('id', 'post,get')) {
	    ...
	    $db->sqlreset()
	        ...
	        ->where('s.sale_status = 4 AND s.projectid !=0');
	...
	if($projectid>0 && $saleid >0 ){
		...
		$add_vehicle =0; 
		$vehicle  = $db->query('SELECT * FROM ' . $db_config['dbsystem']. '.' . $db_config['prefix'] . '_' . $module_data . '_vehicle WHERE saleid = ' . $saleid . ' AND projectid=' . $projectid)->fetch();
		if (empty($vehicle)) {
	        $add_vehicle = 1;
	    } 
	}
		
	$xtpl = new XTemplate('vehicle.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
	...
	if ($show_view && empty($projectid)) {
	    ...
	    while ($view = $sth->fetch()) {
	        ...
	        $view['link_view_vehicle'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=vehicle&amp;saleid=' . $view['id'] . '&amp;projectid=' . $view['projectid'];
	...
	if($projectid>0 && $saleid >0){
		...
		if(!empty($project)){
			if($groups ==1 || $groups == 5 ){
				if($vehicle['status'] ==0){
					$xtpl->parse('main.project.add_vehicle_button');
					$xtpl->parse('main.project.add_vehicle_content');
					$xtpl->parse('main.project.add_vehicle_script');
					
				}elseif($vehicle['status'] ==1 ){
						$xtpl->parse('main.project.edit_vehicle_button');
						$xtpl->parse('main.project.edit_vehicle_content');
						$xtpl->parse('main.project.edit_vehicle_script');	
				}
				
				
				$xtpl->parse('main.project.view_vehicle_content');
				$xtpl->parse('main.project.view_vehicle_script');
				if($vehicle['status'] !=2){
					$xtpl->parse('main.project.view_vehicle_button');
				}
				$xtpl->parse('main.project.show_view_vehicle');
				
				$xtpl->parse('main.project.process_vehicle_script');
			}elseif($groups ==1 || $groups == 23){
				$xtpl->parse('main.project.view_vehicle_button');
				$xtpl->parse('main.project.view_vehicle_content');
				$xtpl->parse('main.project.view_vehicle_script');
			}
