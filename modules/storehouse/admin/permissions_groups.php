...
define("NV_MOD_TABLE",  $db_config['prefix'] . "_users"); 
if($admin_permission->permission->permission_index == '1'){
	$page_title = $lang_global['mod_groups'];
	$contents = '';
	
	// Lay danh sach nhom
	$sql = 'SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_groups WHERE act=1 AND (idsite = ' . $global_config['idsite'] . ' or (idsite=0 AND id>1 AND siteus=1)) ORDER BY idsite,weight ASC';
	$result = $db->query($sql);
	$groupsList = array();
	$groupcount = 0;
	$weight_siteus = 0;
	...
	if ($nv_Request->isset_request('cWeight, id', 'post')) {
	    ...
	    $sql = 'SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_groups WHERE id!=' . $group_id . '  ORDER BY weight ASC';
	    $result = $db->query($sql);
	    ...
	    while ($row = $result->fetch()) {
	        ...
	        $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_groups SET weight=' . $weight . ' WHERE id=' . $row['id'];
	        $db->query($sql);
	    }
	    $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_groups SET weight=' . $cWeight . ' WHERE id=' . $group_id;
	    $db->query($sql);
	...
	if ($nv_Request->isset_request('act', 'post')) {
	    ...
	    $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_groups SET act=' . $act . ' WHERE id=' . $group_id;
	    $db->query($sql);
	...
	if ($nv_Request->isset_request('del', 'post')) {
	    ...
	    $array_groups = array();
	    $result = $db->query('SELECT group_id, userid FROM ' . $db_config['prefix'] . '_' . $module_data . '_users WHERE userid IN (SELECT userid FROM ' . $db_config['prefix'] . '_' . $module_data . '_groups_user WHERE group_id=' . $group_id . ')');
	    ...
	    $db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_groups WHERE id = ' . $group_id);
	    $db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_groups_user WHERE group_id = ' . $group_id);
	
	    // Cập nhật lại thứ tự
	    $sql = 'SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_groups WHERE idsite=' . $global_config['idsite'] . ' ORDER BY weight ASC';
	    $result = $db->query($sql);
	
	    $weight = 0;
	    while ($row = $result->fetch()) {
	        ++$weight;
	        $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_groups SET weight=' . $weight . ' WHERE group_id=' . $row['id'];
	        $db->query($sql);
	    }
	...
	if ($nv_Request->isset_request('gid,approved', 'post')) {
	    ...
	    $db->query('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_groups SET numbers = numbers+1 WHERE id = ' . $gid);
	    ...
	    if (!empty($group_users)) {
	        ...
	        if (empty($type) or $type == 'leaders') {
	            ...
	            if ($numberusers != $groupsList[$group_id]['numbers']) {
	                $db->query('UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_groups SET numbers = ' . $numberusers . ' WHERE id=' . $group_id);
	            }
	        }
	    }
	...
	if ($nv_Request->isset_request('add', 'get') or $nv_Request->isset_request('edit, id', 'get')) {
	    if (defined('NV_IS_SPADMIN')) {
	        ...
	        if ($nv_Request->isset_request('save', 'post')) {
	            ...
	            if (empty($post['id']) or $post['id'] > 30) {
	                
	                
	
	                // Kiểm tra trùng tên nhóm
	                $stmt = $db->prepare('SELECT id FROM ' . $db_config['prefix'] . '_' . $module_data . '_groups WHERE ' . NV_LANG_DATA . '_title LIKE :title AND id!= ' . intval($post['id']) . ' AND (idsite=' . $global_config['idsite'] . ' or (idsite=0 AND siteus=1))');
	                ...
	                if ($nv_Request->isset_request('add', 'get')) {
	                   ...
	                   // print_r($post);die;
	                    ...
	                    WHERE id=" . $post['id']);
						 $stmt->bindParam(':title', $post['title'], PDO::PARAM_STR);
	                    $stmt->bindParam(':email', $post['email'], PDO::PARAM_STR);
	                    $stmt->bindParam(':group_color', $post['group_color'], PDO::PARAM_STR);
	                    $stmt->bindParam(':group_avatar', $post['group_avatar'], PDO::PARAM_STR);
	                    $stmt->bindParam(':config', $post['config'], PDO::PARAM_STR);
