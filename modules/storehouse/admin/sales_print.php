...
if($parentid != $_SESSION[$module_data . '_store_id'] && $_SESSION[$module_data . '_store_id'] > 0|| $groups == 1){
	...
	$export = new NukeViet\StoreHouse\Sales;
		...
		$customer_details = $export->site->getCompanyByID($row['customer_id']);
		$biller_details = $export->site->getUserByID($row['biller_id']);
		...
		$row['product_id'] = 0;
		$row['customer_address'] = $customer_details->address;
		$row['customer_phone'] = $customer_details->phone;
		$row['customer_email'] = $customer_details->email;
		$row['admin_name'] = $biller_details->last_name . ' ' . $biller_details->first_name;
		$row['admin_username'] = $biller_details->username;
		$row['admin_email'] = $biller_details->email;
		...
		$row['status'] = $array_sales_status[$row['sale_status'] ];
	...
	$xtpl = new XTemplate('sales_print.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
