...
if($parentid != $_SESSION[$module_data . '_store_id'] && $_SESSION[$module_data . '_store_id'] > 0 || $groups == 1){
	...
	if (!$nv_Request->isset_request('id', 'post,get')) {
	    ...
	    $db->sqlreset()
	        ->select('COUNT(*)')
	        ->from('' . $db_config['prefix'] . '_' . $module_data . '_sales s')
	        ->join('LEFT JOIN ' . $db_config['prefix'] . '_' . $module_data . '_project p on s.projectid = p.projectid')
	        ->where('sale_status = 1');
	...
	$xtpl = new XTemplate('production_plan.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file);
		$xtpl->assign('LANG', $lang_module);
		$xtpl->assign('NV_LANG_VARIABLE', NV_LANG_VARIABLE);
		$xtpl->assign('NV_LANG_DATA', NV_LANG_DATA);
		$xtpl->assign('NV_BASE_ADMINURL', NV_BASE_ADMINURL);
