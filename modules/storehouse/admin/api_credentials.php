...
$sql = 'SELECT role_id, role_title FROM ' . $db_config['prefix'] . '_' . $module_data . '_api_role ORDER BY role_id DESC';
$result = $db->query($sql);
...
if (empty($global_array_roles)) {
    $url = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=api_roles';
...
$db->sqlreset()->from($db_config['prefix'] . '_' . $module_data . '_api_credential tb1');
...
if ($nv_Request->isset_request('del', 'post')) {
    ...
    $db->query('DELETE FROM ' . $db_config['prefix'] . '_' . $module_data . '_api_credential WHERE credential_ident=' . $db->quote($credential_ident));
...
if ($nv_Request->isset_request('add', 'get') or !empty($credential_ident)) {
    ...
    if ($credential_ident) {
        ...
        $table_caption = $lang_module['api_cr_edit'];
        $array_post = array(
        ...
        $table_caption = $lang_module['api_cr_add'];
        $array_post = array(
            'admin_id' => 0,
            'credential_title' => '',
            'api_roles' => array()
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        if (empty($array_post['credential_title'])) {
            ...
            if (empty($credential_ident)) {
                ...
                while (empty($new_credential_ident) or $db->query('SELECT admin_id FROM ' . $db_config['prefix'] . '_' . $module_data . '_api_credential WHERE credential_ident=' . $db->quote($new_credential_ident))->fetchColumn()) {
                    $new_credential_ident = nv_genpass(32, 3);
                }
                while (empty($new_credential_secret) or $db->query('SELECT admin_id FROM ' . $db_config['prefix'] . '_' . $module_data . '_api_credential WHERE credential_ident=' . $db->quote($new_credential_secret))->fetchColumn()) {
                    $new_credential_secret = nv_genpass(32, 3);
                }

                $sql = 'INSERT INTO ' . $db_config['prefix'] . '_' . $module_data . '_api_credential (
                ...
                // Cập nhật
                $sql = 'UPDATE ' . $db_config['prefix'] . '_' . $module_data . '_api_credential SET
...
if (empty($global_config['remote_api_access'])) {
    ...
    $xtpl->assign('MESSAGE', $lang_module['api_remote_off'] . $url);
...
foreach ($array as $row) {
    ...
    $row['last_access'] = $row['last_access'] ? nv_date('H:i d/m/Y', $row['last_access']) : $lang_module['api_cr_last_access_none'];
