			...
			<div class="controls table-controls">
				<table id="poTable" class="table items table-striped table-bordered table-condensed table-hover sortable_table">
					...
					<tbody class="ui-sortable products">
						...
						<tr id="pro_pc_{product.i}">
							...
							<td> <select class="form-control" name="product[]" id="products_id_{product.i}" ></select>
								<input type="hidden" name="product_id[]" value="{product.id}" class="form-control" id="pro_pc_id_{product.i}" />
								<input type="hidden" name="product_code[]" value="{product.code}" id="pro_pc_code_{product.i}">
								<input type="hidden" name="product_name[]" value="{product.title}" id="pro_pc_name_{product.i}">
								<script type="text/javascript">$(document).ready(function() {product_select("products_id_{product.i}",{product.i},{product.id}); $('#products_id_{product.i}').prop('disabled', true); list_pro_id.push('{product.id}'); }); 
			 </script> 
							</td>
							...
							<td>
								<input type="text" name="product_base_quantity[]"class="form-control" value="{product.quantity}" id="pro_pc_quantity_{product.i}">
								<input type="hidden" name="product_unit[]" value="{product.purchase_unit}"class="form-control" id="pro_pc_unit_{product.i}">
								<script type="text/javascript">$("#pro_pc_quantity_{product.i}").on("change", function () {var quantity_input = $("#pro_pc_quantity_{product.i}").val(); var quantity = Number(quantity_input.replace(/,/gi, "")); var price_input = $("#pro_pc_real_{product.i}").val(); var price = Number(price_input.replace(/,/gi, ""));var total = price*quantity; $("#pro_pc_total_{product.i}").val(number_format(total,0,'.',',')); total_purchase();}); </script>
							</td>
							
							<td>
								<input type="text" name="product_quantity_received[]"class="form-control" value="{product.quantity_received}" id="pro_pc_quantity_receipt_{product.i}">
								<script type="text/javascript">$("#pro_pc_quantity_receipt_{product.i}").on("change", function () {total_purchase();}); </script>
							</td>
							...
							<td>
								<input type="text" name="product_total[]" class="form-control"value=" {product.total}" id="pro_pc_total_{product.i}"> 
							</td>
							<td> </td>
						</tr>
						<!-- END: products -->
					...
					<tfoot>
        				<tr>
        					<td colspan="4">
        						<a href="#" class="btn btn-default btn-xs" onclick="add_product_line(); return false;" >
									<em class="fa fa-plus">&nbsp;</em> {LANG.add_line}
								</a>
								<a href="#" class="btn btn-default btn-xs" onclick="delete_product_line(this); return false;" >
...
<script type="text/javascript">
//<![CDATA[
	$.cookie('products_purchases_total', '{products_purchases_total}' , { path: '/', expires: 1 });
	...
	function total_purchase(){
		...
		for(i=1;i<=pro_pc_total;i++){
			...
			var quantity_receipt = $('#pro_pc_quantity_receipt_' + i).val();
			var subtotal =0;
			var subquantity =0;
