                    ...
                    success: {
                        ...
                        callback: function () {
                            ...
                            if(pos_pin == pos_settings.pin_code) {
                                ...
                                $('#modal-loading').show();
                                window.location.href = site.base_url+"index.php?language=vi&nv=storehouse&op=pos";
...
$('#posdiscount').focus(function () {
    ...
    $(document).on('click', '.edit', function () {
        ...
        if (item.units !== false) {console.log(item.units);
     ...
     * Sell Gift Card modal
     ...
     $(document).on('click', '#addGiftCard', function (e) {
        ...
        $.ajax({
            type: 'get',
            url: site.base_url+'index.php?language=vi&nv=storehouse&op=ajax&mod=sales_sell_gift_card',
            dataType: "json",
            data: { gcdata: gc_data },
	...
	$(document).on('click', '#editCustomer', function (e) {
        ...
        $.ajax({
            type: 'get',
            url: site.base_url+'index.php?language=vi&nv=storehouse&op=ajax&mod=customers_save',
            dataType: "json",
            data: { edcustomer_id: edcustomer_id },
            success: function (data) {
                if(data.result === 'success') {
    ...
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
            ...
            if (pos_settings.remote_printing != 1) {
                ...
                o_items += printLine(product_name("#"+(an-1) + item_name) + ": [ "+ (item_ordered != 0 ? 'xxxx' : formatDecimal(item_qty))) + " ]\n";
                for (var i = 0, len = comments.length; i < len; i++) {
