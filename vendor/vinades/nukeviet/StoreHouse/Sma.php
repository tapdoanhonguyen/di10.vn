   ...
   /*  public function log_payment($type, $msg, $val = null)
     ...
     public function update_award_points($total, $customer, $user = NULL, $scope = null)
    {
        if (!empty($this->Settings->each_spent) && $total >= $this->Settings->each_spent) {
            ...
            $this->db->query('UPDATE ' . $this -> db_prefix . '_' . $this -> mod_data . '_companies SET award_points =' . $total_points . ' WHERE id= ' . $customer);
        }
...
/*     public function paid_opts($paid_by = null, $purchase = false, $empty_opt = false)
    ...
    public function alias($title, $type = NULL, $r = 1)
    {
        //$this->load->helper('text');
        $alias = url_title(convert_accented_characters($title), '-', TRUE);
        $check_alias = $this->site->checkAlias($alias, $type);
        if (!empty($check_alias)) {
            $alias = $alias.$r; $r++;
            $this->alias($alias, $type, $r);
        }
        return $alias;
    } 
